...
app.get('/', (req, res) => {
	...
	request('http://api.petfinder.com/pet.getRandom?format=json&key='+ process.env.API_KEY +'&output=basic', (err, response, body) => {
		if (err) {
...
app.post('/results/search', (req, res) => {
	...
	let searchStr = 'http://api.petfinder.com/pet.find?format=json&key='+ process.env.API_KEY;
...
app.get('/search/:id', (req, res) => {
	...
	if (req.session.location) {
		...
		request('http://api.petfinder.com/breed.list?format=json&key='+ process.env.API_KEY +'&animal='+ species, (err, response, foundBreeds) => {
			if (err) {
...
app.get('/view/pet/:id', (req, res)=>{
	...
	request('http://api.petfinder.com/pet.get?format=json&key='+ process.env.API_KEY +'&id=' + req.params.id, (err, response, foundAnimal) => {
...
app.get('/shelter/search', (req, res) => {
	...
	request('http://api.petfinder.com/shelter.find?format=json&key='+ process.env.API_KEY +'&location=' + req.session.location, (err, response, foundShelters) => {
		if (err) {
			console.error(err)
		}
		else {
...
app.get('/view/shelter/:id', (req, res) => {
	...
	request('http://api.petfinder.com/shelter.get?format=json&key='+ process.env.API_KEY +'&id=' + req.params.id, (err, response, foundShelter) => {
		if (err) {
			console.error(err)
		}
		else {
			...
			request('http://api.petfinder.com/shelter.getPets?format=json&key='+ process.env.API_KEY +'&count=5&id=' + req.params.id, (err, response, foundPets) => {
